  ...
  <div class="w-full p-6">
    ...
    <div class="grid gap-6" style="grid-template-columns: repeat(auto-fill, minmax(260px, 1fr))">
      ...
      >
        <div class="card-body">
          ...
          <div class="grid grid-cols-2 gap-4 border-t border-base-200 pt-4 mt-4">
            ...
            <div>
              <p class="text-xs uppercase font-bold tracking-widest text-base-content/50">Fecha</p>
              <p class="text-lg font-bold">{{ x.fecha }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div
        v-for="n in itemsall"
        :key="n.id"
        class="card bg-warning/10 border border-base-200 shadow-sm hover:shadow-md transition-shadow max-w-md cursor-pointer"
      >
        <div class="card-body" @click="abrirCarpeta(n.id)">
          <div class="flex items-start justify-between mb-2">
            <div class="bg-warning/10 p-4 rounded-xl flex items-center justify-center">
              <i class="fa-regular fa-folder text-2xl"></i>
            </div>
            <div class="dropdown" @click.stop>
              <div @click.stop tabindex="0" role="button" class="btn btn-ghost btn-sm">
          ...
          <div class="grid grid-cols-2 gap-4 border-t border-base-200 pt-4 mt-4">
            <div>
              ...
              <p class="text-lg font-bold">{{ n.reactivos }}</p>
            </div>
            ...
            <div>
              ...
              <p class="text-lg font-bold">{{ n.fecha }}</p>
            </div>
          </div>
        </div>
      </div>
